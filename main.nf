...
process SEQFU_STATS {
    container 'The tools used in this script are seqfu. The official Docker image for seqfu is currently not available, so we can't directly use it. However, there exists a bioinformatics toolkit container (bioconductor/bioconductor_docker) that includes many popular bioinformatics tools like BWA, SAMtools and others. We could potentially use this as a base for our workflows if necessary.'
    tag "$meta.id"
    label 'process_single'
