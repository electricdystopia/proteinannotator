...
process SEQKIT_STATS {
    container 'The script block seems to use `seqkit` for computing statistics of FASTQ files and touch command to create a file. However, without the container directive or specific image name it's hard to provide a recommended Docker image. But assuming the usage of SeqKit (for manipulating sequences in FASTA/Q files) you could use:'
    tag "${meta.id}"
    label 'process_low'
